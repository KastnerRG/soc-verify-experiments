...
extern EXT_C u8 run(Memory_st *restrict mp, void *p_config) {
    ...
    sprintf(f_path, "%sx.bin", TO_STRING(DIR));
    fp = fopen(f_path, "rb");
    debug_printf("DEBUG: Reading from file %s \n", f_path);
    if(!fp) debug_printf("ERROR! File not found: %s \n", f_path);
    bytes = fread(mp->x, 1, sizeof(mem_phy.x), fp);
    fclose(fp);
  #endif
  
  // Start DMA
  set_config(p_config, A_MM2S_0_ADDR , addr_64to32(mem_phy.x));
  set_config(p_config, A_MM2S_0_BYTES,      sizeof(mem_phy.x));
